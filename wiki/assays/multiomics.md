...
> 926 datasets | 2016/04/12 – 2026/04/08

Datasets that simultaneously profile multiple molecular modalities from the same cells or samples in a single experiment — for example, RNA + surface protein (CITE-seq), RNA + chromatin accessibility (10x Multiome, SHARE-seq), or spatial transcriptomics paired with another assay. See individual protocol pages for more specific views.
...
|------|------:|-------------|
| TSV | 505 | Tab-separated data (barcodes, features, fragment files) |
| MTX | 316 | Sparse count matrices (10x CellRanger output, RNA or ATAC) |
| CSV | 308 | Comma-separated data (count matrices, protein counts, metadata) |
| H5 | 248 | HDF5 (CellRanger filtered matrices, combined RNA+ATAC) |
| TXT | 221 | Text files (count matrices, cell metadata) |
| RDS | 92 | R serialized objects (Seurat, MultiAssayExperiment) |
| TAR | 90 | Tar archives (bundled multi-modal outputs) |
| TBI | 88 |  |
| BED | 61 | BED peak calls (ATAC accessibility) |
| XLSX | 35 | Excel (metadata, differential results) |
| H5AD | 28 | AnnData HDF5 (scanpy/Python ecosystem, multi-modal) |
| BW | 24 | BigWig coverage tracks |
| JSON | 23 |  |
| PNG | 23 |  |
| BIGWIG | 21 | BigWig coverage tracks (ATAC or RNA coverage) |
